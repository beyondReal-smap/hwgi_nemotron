...
⚠️ 카피에 없는 보장 항목·보험료·만기 같은 정보를 **만들어내지 마세요**. 그건 본인이 아직 모르는 정보입니다.
- ✓ "보험료가 얼마인지, 어떤 특약이 있는지 궁금하다" (모르는 점은 궁금증으로 표현)
- ✗ "월 3만원에 암 진단비 5천만원이라 합리적이다" (카피에 없는 수치를 지어냄)"""
...
- 더 알고 싶은 점이 있다면, 단정 대신 **궁금증으로** 표현하세요

⚠️ 요약에 없는 상세 약관·특약 한도·면책·보험료를 **만들지 마세요**. 그건 본인이 아직 모르는 정보입니다.
- ✓ "특약 한도와 면책 조건이 어떻게 되는지 약관이 나오면 보고 싶다" (모르는 점은 궁금증으로)
- ✗ "자기부담금 20만원에 갱신형이라 부담스럽다" (요약에 없는 스펙을 지어냄)"""
