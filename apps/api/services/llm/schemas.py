...
1. remaining_query에 메타로 흡수된 단어가 남아 있지 않은가?
2. marital_statuses를 채웠다면 '기혼/미혼/이혼/사별/한부모' 같은 명시 단서가 쿼리에 실제로 있는가?
3. additional_filters의 키·값이 전부 스키마 enum에 글자 그대로 존재하는가?
"""
...
호출 전 점검 (어긋나면 고친 뒤 호출):
1. 객관식이면 answer가 선택지 텍스트와 글자 그대로 일치하는가?
2. answer가 페르소나 성향·형편과 모순되지 않는가?
3. reasoning 50자 이내, confidence 0.0~1.0 범위인가?
...
호출 전 점검 (어긋나면 고친 뒤 호출):
1. 요청 개수와 일치하고, 더블 바렐·유도 질문이 없는가?
2. 객관식 보기가 한 기준으로 MECE이며 2-7개인가? 다른 유형의 options는 빈 배열인가?
3. scale은 양 끝 라벨이 채워졌는가? 기존 질문과 의도가 겹치지 않는가?
"""
...
# 설문 관점(angle) — 매 호출 하나를 무작위 지정해 같은 분야라도 다른 주제로 분기시킨다.
# 시스템 프롬프트의 단일 예시("재방문 요인")로 골격이 수렴하는 것을 깨는 핵심 다양성 레버.
_PLACEHOLDER_ANGLES = [
    "이용·소비 행태와 사용 패턴",
    "해지·이탈·중단을 부르는 원인",
    "신규 가입·구매로 이어지는 결정 동인",
    "가격·요금에 대한 지불 의향과 민감도",
    "온라인·오프라인·앱 등 채널·접점 선호",
    "선택 시 가장 중요하게 보는 기준의 우선순위",
    "아직 충족되지 못한 니즈와 불만 지점",
    "경쟁 대안 대비 갈아탈 의향과 그 이유",
    "상황·맥락(시간대·장소·동반자)별로 달라지는 행태",
    "정보 탐색부터 최종 결정까지의 경로",
    "만족도와 추천·재구매(NPS) 의향",
    "혜택·기능 간 중요도 트레이드오프",
]

# 대상 세그먼트 힌트 — 항상 강제하면 그 자체가 또 다른 틀이 되므로 일부 호출에서만 제시.
...
def _build_placeholder_user_prompt(category: str) -> str:
    """분야 + 무작위 관점·대상 시드를 담은 user 메시지 — 같은 분야라도 매번 다른 주제로 분기.

    분야(category)만으로는 LLM이 시스템 프롬프트 예시의 골격으로 수렴해 결과가 비슷해진다.
    매 호출 관점(필수)·대상(선택)을 무작위 주입해 조합 다양성을 폭발시킨다.
    """
    angle = random.choice(_PLACEHOLDER_ANGLES)
    if random.random() < 0.65:
        seg = random.choice(_PLACEHOLDER_SEGMENTS)
        seg_line = (
            f"가능하면 '{seg}' 같은 대상을 염두에 두되, 이 분야에 더 자연스러운 다른 대상이 "
            f"있으면 그쪽을 택해도 됩니다. "
        )
    else:
        seg_line = "대상 세그먼트는 이 분야에 가장 잘 맞는 것으로 직접 좁혀 잡으세요. "
    return (
        f"이번에는 '{category}' 분야의 설문 예시 1세트를 만들어 "
        f"submit_survey_placeholder 도구로 제출하세요. "
        f"이번 설문의 관점은 '{angle}'입니다 — 이 각도에서 주제를 잡으세요. "
        f"{seg_line}"
        f"예시의 '재방문/이용 행태 요인 파악' 같은 틀을 그대로 따르지 말고, 위 관점에 맞는 "
        f"신선한 주제로 잡으세요. "
